# quiz_firestore_app_final_en.py
import os
import re
import json
import uuid
...
You will be given messy exam questions (English only) with options. 
    ...
    if quizzes_list:
        ...
        for idx, q in enumerate(current_questions, start=start+1):
            ...
            if selected_option:
                ...
                selected_index = None
            choice = st.radio(f"Answer for Q{idx}", options=display_options, key=f"q{idx}", index=selected_index)
